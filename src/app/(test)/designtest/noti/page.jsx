...
import { query, where, getDocs, collection, serverTimestamp, setDoc, doc } from 'firebase/firestore'
...
// Server Actions
import { batchAddisConfirmed, deletePostById, readFromFirestore, getParentEngTitlesFromCurrentSubscriber, postToFirestore } from '@/action/google/firestore'
...
const NotiAdmin = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Load notiTemplates
      const notiTemplatesResp = await readFromFirestore("templates")
      setNotiTemplates(JSON.parse(notiTemplatesResp.posts))
              ...
              <Tr key={row.id}>
                ...
                <Td>
                  <NextLinkButton
                    colorScheme='blue'
                    isDiabled
                    href={`/designtest/noti/${row.original.notificationId}`}
                    >
                      集計表示
                  </NextLinkButton>
                </Td>
